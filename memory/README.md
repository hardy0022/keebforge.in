...
- **Complete:** Foundation (Phase 3), all content pages (Phase 4), full security/pricing/SEO audit (Phase 4.5), shop + cart (Phase 5), Better Auth migration (replaces Supabase Auth), admin Phase 1 (auth + login, sidebar shell, dashboard, orders list/detail, tracking cache), product/shop management Phase 1+2 (categories/brands/products CRUD, variants, inventory ledger, CSV export/import, dashboard product metrics), customer sign-in with Google/Discord OAuth + email (`/login`, `/api/auth/me`, `/auth/callback`), single `/repair` page (Electronics Repair card + inquiry form with optional photo upload → Cloudinary, replacing the per-device repair pricing pages — D-019).
- **Verified:** `typecheck`, `lint`, `build` all green; admin routes 200 when authenticated and 307→`/login` when not; shop + product pages render seeded catalog; server actions for status/inventory/variants/categories/brands; export CSV; auth sign-up/sign-in/session/sign-out end-to-end against the live DB; `/api/auth/dash/validate` → 401 locally (dash plugin live); `sendInquiry` image validation (type/size/count, no-Cloudinary graceful error) via route-handler test — actual Cloudinary upload unverified (no creds).
...
- **Blocked / pending:** Cloudinary integration needs real API credentials (upload route returns 501 until set); Razorpay keys are placeholders (checkout payment in Phase 6); **Dash ownership verification blocked on a Vercel domain-level apex→www 308 redirect** (fix = Vercel dashboard Domains + Production env + redeploy — see `08-roadmap.md`); Google/Discord OAuth production callback URIs must be registered in the provider consoles; customer shop Phase 3 (variant picking, brand pages, wishlist, reviews submission) and checkout Phase 4 remain.
- **Current homepage:** full homepage restored (D-018 superseded) — Hero, marquee, DB reviews marquee, services, FAQ, CTA. Note: the `/reviews` route listed in `05-routes-seo.md` does NOT exist in the codebase (pruned with the maintenance swap, not yet restored) — do not link to it.
