...
function productLabel(r: {
  ...
  return live
    ? { label, href: `/product/${live.slug}` }
    : slug
      ? { label, href: `/product/${slug}` }
      : { label };
}
...
export default async function AdminReviewsPage({
  ...
  const result = await getAdminReviews({
    status: STATUSES.includes(sp.status as never)
      ...
      : undefined,
    type: TYPES.includes(sp.type as never)
      ? (sp.type as (typeof TYPES)[number])
      : undefined,
    rating:
      sp.rating && !Number.isNaN(Number(sp.rating))
        ? Number(sp.rating)
        : undefined,
    q: sp.q,
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <input
          ...
          name="q"
          defaultValue={sp.q}
          placeholder="Search title, body, author or product"
      ...
      {result.items.length === 0 ? (
        <div className="empty">
          ...
          New customer reviews will appear here for moderation before they go
          live.
        </div>
      ) : (
                    ...
                    <tr key={r.id}>
                      ...
                      <td>
                        <div style={{ fontWeight: 600, fontSize: "0.8rem" }}>
                          {r.profile?.name ?? r.authorName ?? "Guest"}
                        </div>
                        {r.profile?.email && (
                          <div
                            className="muted num"
