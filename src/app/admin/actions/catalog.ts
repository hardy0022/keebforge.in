 ...
 */
function toOptionConfig(v: FormDataEntryValue | null): {
  error?: string;
  ...
  if (!Array.isArray(raw))
    return { error: "Option configuration is malformed.", groups: [] };
  if (raw.length === 0) return { groups: [] };
  if (raw.length > MAX_OPTION_GROUPS)
    return {
      error: `At most ${MAX_OPTION_GROUPS} option groups are allowed.`,
      groups: [],
    };
  ...
  for (const row of raw) {
    ...
    if (groupNames.has(name.toLowerCase()))
      return { error: `Duplicate option group "${name}".`, groups: [] };
    groupNames.add(name.toLowerCase());

    if (!Array.isArray(r.options) || r.options.length === 0) {
      return {
        error: `Option group "${name}" needs at least one option.`,
        groups: [],
      };
    }
    ...
    for (const o of r.options) {
      ...
      if (!oName)
        return { error: `Every option in "${name}" needs a name.`, groups: [] };
      if (optionNames.has(oName.toLowerCase()))
        return {
          error: `Duplicate option "${oName}" in "${name}".`,
          groups: [],
        };
      optionNames.add(oName.toLowerCase());
      const addon =
        typeof or.addon === "number" &&
        Number.isInteger(or.addon) &&
        or.addon >= 0
          ? or.addon
          : -1;
      if (addon < 0)
        return {
          error: `Add-on price for "${oName}" must be zero or more.`,
          groups: [],
        };
      options.push({
...
export async function saveProduct(
  ...
  const slug = d.slug || slugify(d.name, { stripQuotes: true });
  ...
  const productType = ["CUSTOM", "NEW", "CLEARANCE"].includes(
    d.productType ?? "",
  )
    ? (d.productType as never)
    : undefined;
  const condition = [
    "NEW",
    "OPEN_BOX",
  ...
  const data: Prisma.ProductUncheckedCreateInput = {
    ...
    features: d.features
      ? { list: toList(formData.get("features")) }
      : Prisma.JsonNull,
    whatsIncluded: d.whatsIncluded
      ? { list: toList(formData.get("whatsIncluded")) }
      : Prisma.JsonNull,
    ...
    cardFeatures: cardFeatures.length
      ? (cardFeatures as Prisma.InputJsonValue)
      : Prisma.JsonNull,
    price: Math.round(d.price * 100),
    compareAtPrice: d.compareAtPrice
      ? Math.round(d.compareAtPrice * 100)
      : null,
  ...
  try {
    ...
    try {
      uploaded = JSON.parse(
        String(formData.get("uploadedAssets") || "[]"),
      ) as UploadedAsset[];
    } catch {
      uploaded = [];
    }
    ...
    for (const asset of uploaded) {
      ...
      const moved = await renameAsset(
        asset.publicId,
        `keebforge/products/${product.id}/${base}`,
      );
      if (moved) {
        byUrl.set(asset.url, moved);
      } else {
        console.error(
          `[catalog] draft asset stuck in drafts folder: ${asset.publicId}`,
        );
      }
    }
    ...
    if (newImages.length > 0) {
      ...
      if (removed.length > 0) {
        ...
        for (const r of removed) {
          if (r.publicId && !(await deleteImage(r.publicId))) {
            console.error(
              `[catalog] orphaned product asset after removal: ${r.publicId}`,
            );
          }
        }
      }
      ...
      await prisma.productImage.updateMany({
        where: { productId: product.id, url: primary.url },
        data: { primary: true },
      });
    }
    ...
    if (
      optionConfig.groups.length > 0 ||
      (await prisma.productOptionGroup.count({
        where: { productId: product.id },
      })) > 0
    ) {
...
export async function duplicateProduct(
  ...
  const src = await prisma.product.findUnique({
    where: { id },
    include: { images: true, optionGroups: { include: { options: true } } },
  });
...
export async function saveBrand(
  ...
  if (!parsed.success)
    return { error: parsed.error.issues[0]?.message ?? "Invalid brand." };
  const d = parsed.data;
  const slug = d.slug || slugify(d.name, { stripQuotes: true });
  ...
  const parsedOptions = Object.fromEntries(
    d.options
      ...
      .map((l) => {
        ...
        return eq === -1
          ? null
          : ([l.slice(0, eq).trim(), l.slice(eq + 1).trim()] as [
              string,
              string,
            ]);
      })
  ...
  const variant = await prisma.productVariant.upsert({
    ...
    update: {
      ...
      compareAtPrice: d.compareAtPrice
        ...
        : null,
      weight: d.weight || null,
      options: Object.keys(parsedOptions).length
        ? parsedOptions
        : Prisma.JsonNull,
      active: d.active === "on" || d.active === undefined,
    },
...
export async function deleteVariant(
  ...
  const orderUsage = await prisma.orderItem.count({
    where: { variantInfo: { path: ["variantId"], equals: id } },
  });
...
export async function adjustInventory(
  ...
  if (!profile)
    return {
      error:
        "Your account has no staff profile. Sign in through the site once, then retry.",
    };
...
export async function importProducts(
  ...
  for (const row of rows.slice(1)) {
    ...
    const slug = (
      row[idx("slug")]?.trim() || slugify(name, { stripQuotes: true })
    ).toLowerCase();
  ...
  return {
    ok: true,
    message: `Imported ${created} product${created === 1 ? "" : "s"}, skipped ${skipped}.`,
  };
}
