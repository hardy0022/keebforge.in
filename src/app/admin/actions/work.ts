...
export async function saveWork(
  ...
  const workPerformed =
    String(formData.get("workPerformed") ?? "").trim() || null;
  ...
  const sortOrder = Math.max(
    0,
    Math.floor(Number(formData.get("sortOrder")) || 0),
  );
  ...
  if (!Array.isArray(imageState) || imageState.length > MAX_IMAGES)
    return { error: `At most ${MAX_IMAGES} images per project.` };

  let removedPublicIds: string[] = [];
  try {
    const r = JSON.parse(String(formData.get("removedImages") ?? "[]"));
    if (Array.isArray(r))
      removedPublicIds = r.filter((x): x is string => typeof x === "string");
  } catch {
  ...
  if (slug) {
    const clash = await prisma.workProject.findFirst({
    ...
    if (clash)
      return {
        error:
          "A project with that slug already exists — use a different title or slug.",
      };
  }
  ...
  for (const entry of imageState) {
    ...
    if (typeof entry.fileIndex !== "number")
      return { error: "Image data is malformed." };
    const file = formData.get(`file-${entry.fileIndex}`);
    if (!(file instanceof File) || file.size === 0) continue;
    if (file.size > MAX_BYTES)
      return { error: "One of the images exceeds the 8 MB limit." };
  ...
  for (const publicId of removedPublicIds) {
    await deleteImage(publicId).catch((e) =>
      console.error(`[work] delete asset failed: ${publicId}`, e),
    );
  }
...
export async function toggleWork(
  projectId: string,
  active: boolean,
): Promise<ActionState> {
  await requirePermission("setting", "update");
...
export async function moveWork(
  ...
  await requirePermission("setting", "update");
  const all = await prisma.workProject.findMany({
  ...
  if (to < 0 || to >= all.length)
    return { error: "Already at the edge of the list." };
  const ordered = [...all];
...
export async function deleteWork(projectId: string): Promise<ActionState> {
  ...
  for (const img of images) {
    if (img.publicId)
      await deleteImage(img.publicId).catch((e) =>
        console.error(`[work] delete asset failed: ${img.publicId}`, e),
      );
  }
  for (const path of CLOSE_PATHS) revalidatePath(path);
  revalidatePath("/admin/work");
