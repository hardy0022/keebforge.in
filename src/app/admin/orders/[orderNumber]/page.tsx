...
function orderSubtitle(order: {
  ...
  const devKey =
    typeof s.deviceType === "string"
      ? s.deviceType
      : order.repairs[0]?.deviceType;
  const device = (DEVICE_LABEL[devKey] ?? "").toLowerCase();
  switch (order.type) {
    case "REPAIR": {
      const serviceType =
        typeof s.serviceType === "string" ? s.serviceType : "";
      const prefix =
        serviceType === "custom"
          ? "Custom"
          : serviceType && serviceType !== "repair"
            ...
            : "";
      return (
        [prefix, device && "repair" ? `${device} repair` : null]
          .filter(Boolean)
          .join(" ") || "Repair order"
      );
    }
...
function serviceDeviceRows(summary: unknown): Array<[string, string]> {
  ...
  if (typeof s.switchModel === "string" && s.switchModel)
    rows.push(["Switch model", s.switchModel]);
...
function isQuoteOrder(summary: unknown): boolean {
  return Boolean(
    ...
    (summary as { hasQuotes?: unknown }).hasQuotes === true,
  );
}
...
function isQuoteService(summary: unknown, slug: string): boolean {
  if (!summary || typeof summary === "object") {
    const lines = (
      summary as {
        services?: Array<{ slug?: unknown; isQuote?: unknown }>;
      } | null
    )?.services;
...
function repairNotes(notes: string | null): {
  ...
  try {
    ...
    return {
      ...
      workTypes: Array.isArray(o.workTypes)
        ? o.workTypes.filter((w): w is string => typeof w === "string")
        : undefined,
...
export default async function AdminOrderDetail({
  ...
  const statuses = (
    Object.entries(ORDER_STATUS_LABELS) as [string, string][]
  ).map(([value, label]) => ({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
        ...
        <OverviewCard title="Payment">
          <dl className="pill-grid">
            ...
            <div className="kv">
              <dt>Status</dt>
              <dd>
                <span className={`badge ${payBadge(order.paymentStatus)}`}>
            ...
            {configured && (
              <div className="kv">
                ...
                <dd
                  className="num"
                  style={{ color: balance > 0 ? "var(--warn)" : "var(--ok)" }}
                >
                  {formatINR(balance)}
                </dd>
              </div>
      ...
      <div className="admin-card" id="items">
        ...
        >
          ...
          <span className="muted num">
            Total{" "}
            <b style={{ fontSize: "0.95rem", color: "var(--t1)" }}>
              {formatINR(order.total)}
            </b>
          </span>
        </div>
        <div style={{ marginTop: 12 }}>
          {!hasLines ? (
            <div style={{ color: "var(--t3)", fontSize: "0.85rem" }}>
              No line items
            </div>
          ) : (
              ...
              <table className="admin-table">
                ...
                <tbody>
                  ...
                  {order.services.map((s) => {
                    const quote =
                      s.lineTotal === 0 &&
                      isQuoteService(order.summary, s.slug);
                    return (
                      <tr key={s.id}>
                        <td>
        ...
        {order.repairs.map((r) => {
          ...
          const workTypes = info.workTypes?.length
            ? info.workTypes
            : Array.isArray(summary.workTypes)
              ...
              : [];
          const brand =
            typeof summary.brand === "string" ? summary.brand : undefined;
          const model =
            typeof summary.model === "string" ? summary.model : undefined;
          const serviceType =
            info.serviceType ??
            (typeof summary.serviceType === "string"
              ? summary.serviceType
              : undefined);
          const condition =
            info.condition ??
            (typeof summary.condition === "string" ? summary.condition : null);
          const budget =
            info.budget ??
            (typeof summary.budget === "string" ? summary.budget : null);
          return (
            <div
              key={r.id}
              style={{
              ...
              <dl className="pill-grid" style={{ marginTop: 8 }}>
                ...
                {workTypes.length > 0 && (
                  <div className="kv" style={{ gridColumn: "1 / -1" }}>
                    ...
                    >
                      {workTypes.map((w) => (
                        <span key={w} className="badge badge-lime">
                          {w}
                        </span>
                      ))}
                    </dd>
        ...
        {order.type === "SERVICE" &&
          serviceDeviceRows(order.summary).length > 0 && (
            <div
              style={{
                borderTop: "1px dashed var(--bdr)",
                marginTop: 12,
      ...
      <div className="admin-card" id="financial">
        ...
        >
          ...
          >
            <div className="kv">
              <dt>Subtotal</dt>
              <dd className="num">{formatINR(order.subtotal)}</dd>
            </div>
            <div className="kv">
              <dt>Shipping</dt>
              <dd className="num">{formatINR(order.shipping)}</dd>
            </div>
            {order.discount > 0 && (
              <div className="kv">
                <dt>Discount</dt>
                <dd className="num">−{formatINR(order.discount)}</dd>
              </div>
            )}
            ...
            <div
              style={{ borderTop: "1px solid var(--bdr)", margin: "4px 0" }}
            />
            <div className="kv">
              <dt>Paid</dt>
              <dd className="num">{formatINR(payTotal)}</dd>
          ...
          <div style={{ flex: "1 1 320px", minWidth: 260 }}>
            {!configured ? (
              ...
              <span className="badge badge-ok" style={{ marginBottom: 10 }}>
                ✓ Paid {formatINR(payTotal)}
              </span>
            ) : (
              <span className="badge badge-warn" style={{ marginBottom: 10 }}>
              ...
              >
                ...
                {order.payments.map((p) => (
                  <div key={p.id} className="kv">
                    ...
                    <dd className="num">
                      {formatINR(p.amount)}{" "}
                      <span className={`badge ${payBadge(p.status)}`}>
                        {p.status}
                      </span>
                    </dd>
                  </div>
                        ...
                        .map((p) => (
                          ...
                          >
                            ...
                            <div className="flex justify-between">
                              ...
                              <code style={{ color: "var(--t1)" }}>
                                {p.razorpaySignature
                                  ? `${p.razorpaySignature.slice(0, 16)}…`
                                  : "—"}
                              </code>
                            </div>
                          </div>
      ...
      <div
        ...
        style={{ gridTemplateColumns: "1.6fr 1fr", gap: 18 }}
      >
        <div className="admin-card" id="timeline">
          ...
          {order.timeline.length === 0 ? (
            <p className="muted" style={{ marginTop: 12 }}>
              No timeline entries yet — add the first one above.
            </p>
          ) : (
              ...
              {order.timeline.map((t, idx) => {
                ...
                return (
                  <div key={t.id} className="tl-item">
                    <span
                      className={`badge ${latest ? "badge-lime" : statusBadge(t.status)}`}
        ...
        <div className="admin-card" id="notes">
          ...
          {order.messages.length === 0 ? (
            <p className="muted" style={{ marginTop: 12 }}>
              No notes yet — record a note above.
            </p>
          ) : (
            <div
              style={{
      ...
      <div className="admin-card" id="delivery">
            ...
            declaredValuePaise: Math.max(
              ...
              order.type === "REPAIR"
                ? Math.max(
                    ...
                    Math.round(
                      Number(
                        (order.summary as { budget?: unknown } | null)
                          ?.budget ?? 0,
                      ) * 100,
                    ) || 0,
                  )
        ...
        >
          ...
          {order.shippingAddress && (
            <div
              style={{ fontSize: "0.84rem", lineHeight: 1.6, margin: "10px 0" }}
            >
