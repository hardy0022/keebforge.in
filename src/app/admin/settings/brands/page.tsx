...
export const metadata: Metadata = {
  title: "Brands | KeebForge Admin",
  robots: { index: false, follow: false },
};

export default async function AdminSettingsBrandsPage({
  searchParams,
}: {
  ...
  const editingBrand = editBrandId
    ? (brands.find((b) => b.id === editBrandId) ?? null)
    : null;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div>
        ...
        <p className="muted" style={{ marginTop: 2 }}>
          Reusable brand catalogue — products reference brands here.
        </p>
      </div>
            ...
            <tbody>
              {brands.map((b) => (
                <tr key={b.id}>
                  ...
                  <td>
                    <span
                      className={`badge ${b.active ? "badge-ok" : "badge-err"}`}
                    >
                    ...
                    <Link
                      href={
                        editingBrand?.id === b.id
                          ...
                          : `/admin/settings/brands?brand=${b.id}`
                      }
