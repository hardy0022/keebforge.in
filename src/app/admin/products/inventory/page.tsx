    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div>
        ...
        <p className="muted" style={{ marginTop: 2 }}>
          Stock, reservations and adjustments — every change writes a
          transaction.
        </p>
      </div>
...
function ProductRow({
  ...
  const vBadges = (v: { stock: number; reservedQuantity: number }) => {
    ...
    return {
      cls: avail > 0 ? "badge-ok" : "badge-err",
      label: avail > 0 ? `${avail} available` : "Out",
    };
  };
  return (
    <>
      ...
      {product.variants.map((v) => {
        ...
        return (
          <tr key={v.id} style={{ opacity: 0.85 }}>
            <td style={{ paddingLeft: 28, color: "var(--t2)" }}>
              ↳ {product.name}
            </td>
            <td>{v.name}</td>
            ...
            <td className="num">
              {availableStock(v.stock, v.reservedQuantity)}
            </td>
            <td className="num muted">{product.lowStockThreshold}</td>
            <td>
            ...
            <td>
              <details>
                ...
                <div style={{ marginTop: 8, minWidth: 300 }}>
                  <InventoryForm productId={product.id} variantId={v.id} />
                </div>
              </details>
            </td>
          </tr>
