...
import {
  ...
  VariantsManager,
  InventoryForm,
} from "@/components/admin/products/ProductDetailClient";

export const metadata: Metadata = {
  title: "Product | KeebForge Admin",
...
export default async function AdminProductDetail({
  ...
  const features = Array.isArray(product.features)
    ...
    : product.features &&
      ...
      ? (product.features as { list: string[] }).list
      : [];
  const included =
    product.whatsIncluded &&
    typeof product.whatsIncluded === "object" &&
    "list" in product.whatsIncluded
      ? (product.whatsIncluded as { list: string[] }).list
      : [];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        ...
        <div className="admin-actions">
          ...
          <span
            className={`badge ${product.status === "ACTIVE" ? "badge-lime" : product.status === "DRAFT" ? "badge-warn" : "badge-err"}`}
          >
            {PRODUCT_STATUS_LABELS[product.status]}
          </span>
          {product.featured && (
            <span className="badge badge-lime">Featured</span>
          )}
      ...
      >
        ...
        >
          <div className="admin-card">
            ...
            <div className="kv">
              <dt>SKU</dt>
              <dd className="num">{product.sku ?? "—"}</dd>
            </div>
            <div className="kv">
              <dt>Barcode</dt>
          ...
          <div className="admin-card">
            ...
            <div className="kv">
              <dt>Selling price</dt>
              <dd className="num">{formatINR(product.price)}</dd>
            </div>
            {product.compareAtPrice != null && product.compareAtPrice > 0 && (
              <div className="kv">
                <dt>Compare-at</dt>
                <dd className="num">{formatINR(product.compareAtPrice)}</dd>
              </div>
            )}
            <div className="kv">
              <dt>Cost price</dt>
              <dd className="num">
                {product.costPrice != null ? formatINR(product.costPrice) : "—"}
              </dd>
            </div>
            <div className="kv">
              <dt>GST rate</dt>
              <dd className="num">{product.gstRate}%</dd>
            </div>
            <div className="kv">
              <dt>Margin</dt>
              <dd className={`num ${margin >= 0 ? "" : "muted"}`}>
                {product.costPrice != null ? formatINR(margin) : "—"}
              </dd>
            </div>
          </div>
          ...
          <div className="admin-card">
            ...
            <div className="kv">
              ...
              <dd>{product.shippingClass ?? "—"}</dd>
            </div>
            <div className="kv">
              <dt>Free shipping</dt>
              <dd>{product.freeShipping ? "Yes" : "No"}</dd>
            </div>
        ...
        >
          <div className="admin-card">
            ...
            <div className="kv">
              <dt>Stock</dt>
              <dd className="num">{product.stock}</dd>
            </div>
            <div className="kv">
              <dt>Reserved</dt>
              <dd className="num">{product.reservedQuantity}</dd>
            </div>
            <div className="kv">
              <dt>Available</dt>
              <dd className="num">
            ...
            <div className="kv">
              ...
              <dd className="num">{product.lowStockThreshold}</dd>
            </div>
            <div className="kv">
              <dt>Backorders</dt>
              <dd>{product.allowBackorders ? "Allowed" : "Not allowed"}</dd>
            </div>
            <div className="kv">
              <dt>Tracking</dt>
              <dd>{product.inventoryTracking ? "Enabled" : "Disabled"}</dd>
            </div>
          </div>
          ...
          <div className="admin-card">
            ...
            <div className="kv">
              <dt>Units sold</dt>
              <dd className="num">{salesUnits}</dd>
            </div>
            <div className="kv">
              <dt>Revenue</dt>
              <dd className="num">{formatINR(salesRevenue)}</dd>
            </div>
          </div>
            ...
            {product.reviews.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {product.reviews.slice(0, 5).map((r) => (
                  ...
                  >
                    <span
                      className={`badge ${r.status === "APPROVED" ? "badge-ok" : r.status === "REJECTED" ? "badge-err" : "badge-warn"}`}
                    >
                      {r.status}
                    </span>{" "}
                    <b>
      ...
      <div className="admin-card">
        ...
        {product.images.length === 0 ? (
          <div className="empty">
            <b>No images</b>Add images from the Edit page.
          </div>
        ) : (
      ...
      <div className="admin-card" style={{ padding: 8 }}>
        ...
        {product.orderItems.length === 0 ? (
          <div className="empty">
            <b>No orders yet</b>Sales will appear here after checkout is live.
          </div>
        ) : (
              ...
              <tbody>
                {product.orderItems.map((oi) => (
                  <tr key={oi.id}>
                    <td>
                      <Link
                        href={`/admin/orders/${oi.order.orderNumber}`}
                    ...
                    <td className="muted num">
                      {fmtIST(oi.order.createdAt, {
                        day: "2-digit",
                        month: "short",
                      })}
                    </td>
                    <td>
                      <span className="badge">{oi.order.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="admin-card">
        ...
        {product.inventoryMovements.length === 0 ? (
          <div className="empty">
            <b>No inventory movements yet</b>
          </div>
        ) : (
          <div style={{ overflowX: "auto" }}>
            <table className="admin-table">
              ...
              <tbody>
                {product.inventoryMovements.map((m) => (
                  <tr key={m.id}>
                    <td
                      className={`num ${m.delta > 0 ? "" : ""}`}
                      style={{
                        color: m.delta > 0 ? "var(--ok)" : "var(--err)",
                      }}
                    >
