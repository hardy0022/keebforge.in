...
export default async function AdminProductsPage({
  ...
  const stockBadge = (p: {
    ...
    p.available > 0
      ? { cls: "badge-ok", label: `In stock (${p.available})` }
      : p.stock > 0 && p.stock <= p.lowStockThreshold
        ? { cls: "badge-warn", label: `Low stock (${p.available})` }
        : { cls: "badge-err", label: "Out of stock" };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div>
          ...
          <p className="muted" style={{ marginTop: 2 }}>
            Manage KeebForge shop inventory and products.
          </p>
        </div>
        <div className="admin-actions">
          ...
          <Link href="/admin/products/export" className="btn-admin">
            Export Products
          </Link>
          <Link href="/admin/products/import" className="btn-admin">
            Import Products
          </Link>
        </div>
      </div>
      ...
      >
        ...
        <select
          className="select"
          name="status"
          defaultValue={sp.status ?? "any"}
          style={{ flex: "0 1 150px" }}
        >
          {STATUSES.map((s) => (
            <option key={s} value={s}>
              {s === "any" ? "All statuses" : PRODUCT_STATUS_LABELS[s as never]}
            </option>
          ))}
        </select>
        <select
        ...
        <div className="admin-actions" style={{ marginLeft: "auto" }}>
          ...
          {Object.keys(sp).length > 0 && (
            <Link href="/admin/products" className="btn-admin">
              Clear
            </Link>
          )}
        </div>
                ...
                {result.items.map((p) => {
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td>
                        <span
                          className={`badge ${p.status === "ACTIVE" ? "badge-lime" : p.status === "DRAFT" ? "badge-warn" : "badge-err"}`}
                        >
