...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "[shipping/calculate] product(s) missing shipping weight:",
        items.map((i) => ({ id: i.productId, name: i.product.name })),
      );
    ...
    const priority: ShippingErrorCode[] = [
      "PINCODE_UNAVAILABLE",
      "RATE_LIMITED",
      "UPSTREAM_ERROR",
      "INVALID_CREDENTIALS",
