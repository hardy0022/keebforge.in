...
export function ReviewPagination({
  ...
  return (
    <nav className="reviews-pagination" aria-label="Review pages">
      {current > 1 ? (
        <Link
          className="reviews-page reviews-page-arrow"
          href={`${basePath}?rp=${current - 1}`}
        ...
        <span
          className="reviews-page reviews-page-arrow is-disabled"
          aria-hidden="true"
        >
          ←
        </span>
      ...
      {nums.map((n, i) =>
        n === null ? (
          <span
            key={`e${i}`}
            className="reviews-page-ellipsis"
            aria-hidden="true"
          >
            …
          </span>
