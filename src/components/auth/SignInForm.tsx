...
export function SignInForm({ redirectTo = "/" }: { redirectTo?: string }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {enabled.map((p) => (
        <button key={p.id} type="button" className="btn-social" onClick={() => social(p.id as "google" | "discord")}>
          {p.icon}
      ...
      <form onSubmit={onSubmit} className="form-row" style={{ margin: 0 }}>
        ...
        />
        {error && (
          <p className="muted" style={{ color: "var(--err)", fontSize: "0.8rem", margin: 0 }}>
            {error}
          </p>
        )}
