  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-end" }}>
      <div className="admin-actions" style={{ justifyContent: "flex-end" }}>
        <button type="button" className={`btn-admin ${open ? "" : "primary"}`} onClick={() => setOpen((o) => !o)}>
          Update Status
        </button>
        <details className="order-more">
          ...
          <div className="order-more-menu">
            ...
            <a href="#danger" style={{ color: "var(--err)" }}>Danger zone</a>
          </div>
        </details>
      </div>
      {open && (
        <div className="admin-card" style={{ padding: 14, width: "min(560px, 100%)" }}>
            ...
            {(pending) => (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <div className="admin-actions" style={{ alignItems: "center" }}>
                  ...
                  <button type="button" className="btn-admin" onClick={() => setOpen(false)}>
                    Close
                  </button>
                </div>
                <input name="note" className="input" placeholder="Note for the timeline (optional)" disabled={pending} />
              </div>
            )}
          </ActionForm>
        </div>
