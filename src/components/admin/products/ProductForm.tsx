...
const STATUSES = Object.entries(PRODUCT_STATUS_LABELS) as [
...
export function ProductForm({
  ...
  const [primaryIdx, setPrimaryIdx] = useState(
    product?.images.findIndex((i) => i.primary) ?? 0,
  );
  ...
  const [draftToken] = useState(
    () => Math.random().toString(36).slice(2, 10) + Date.now().toString(36),
  );
  const [cardFeats, setCardFeats] = useState<CardFeature[]>(() =>
    (product?.cardFeatures ?? [])
      .slice(0, MAX_CARD_FEATURES)
      .map((f) => ({ icon: f.icon, label: f.label, value: f.value })),
  );
  ...
  const specsLines = product?.specifications
      ...
      .map((g) => ({
        ...
        options: g.options
          ...
          .map((o) => ({
            id: o.id,
            name: o.name.trim(),
            addon: Math.round((parseFloat(o.addon) || 0) * 100),
          })),
      }))
  ...
  const setOption = (
    gi: number,
    oi: number,
    patch: Partial<OptionGroupState["options"][number]>,
  ) =>
    setOptGroups((rows) =>
      rows.map((g, i) =>
        i === gi
  ...
  const addImage = () =>
    setImages((imgs) => [
      ...imgs,
      { url: "", alt: "", sortOrder: imgs.length, primary: false },
    ]);
  ...
  const setImage = (idx: number, patch: Partial<ImageProp>) =>
    setImages((imgs) =>
      imgs.map((im, i) => (i === idx ? { ...im, ...patch } : im)),
    );
  ...
  async function removeImage(idx: number) {
    ...
    if (
      img?.publicId &&
      uploadedAssets.some((a) => a.publicId === img.publicId)
    ) {
      setUploadedAssets((rows) =>
        rows.filter((a) => a.publicId !== img.publicId),
      );
      await fetch("/api/uploads?publicId=" + encodeURIComponent(img.publicId), {
        method: "DELETE",
      }).catch(() => {});
    }
  }
  ...
  function uploadFiles(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      ...
      xhr.upload.onprogress = (e) =>
        e.lengthComputable &&
        setUploadPct(
          Math.round(((done + e.loaded / e.total) / files.length) * 100),
        );
      xhr.onload = () => {
        done += 1;
        try {
          ...
          if (!statusOk || !data.ok || !data.url || !data.publicId) {
            setUploadError(
              data.error ?? "Unable to upload image. Please try again.",
            );
          } else {
        ...
        setUploadPct(
          done >= files.length ? null : Math.round((done / files.length) * 100),
        );
      };
      xhr.onerror = () => {
        done += 1;
  ...
  return (
    ...
    >
      ...
      {state.error && !state.ok && (
        <p style={{ color: "var(--err)", fontSize: "0.85rem" }}>
          {state.error}
        </p>
      )}
      ...
      <Section title="1 · Basic information">
        ...
        >
          ...
          <Field
            label="Category"
            hint="Database-driven — manage under Categories."
          >
            <select
              className="select"
              name="categoryId"
          ...
          <Field
            label="Product type"
            hint="Where it lives in the shop: custom, in-stock or clearance."
          >
            <select
              className="select"
              name="productType"
              defaultValue={product?.productType ?? "NEW"}
              disabled={pending}
            >
      ...
      <Section title="2 · Pricing">
        ...
        >
          <Field
            label="Selling price (₹)"
            hint="What customers pay. Stored in paise."
          >
            <input
              className="input"
              type="number"
          ...
          <Field
            ...
            hint="Shown struck-through. Leave 0 for no sale."
          >
            <input
              className="input"
              type="number"
              name="compareAtPrice"
              defaultValue={product ? (product.compareAtPrice ?? 0) / 100 : ""}
              min={0}
              step="1"
              disabled={pending}
            />
          ...
          >
            <input
              ...
              type="number"
              name="costPrice"
              defaultValue={product ? (product.costPrice ?? 0) / 100 : ""}
              min={0}
              step="1"
              disabled={pending}
            />
          ...
          <Field
            label="GST rate (%)"
            hint="Applied at checkout for taxable goods."
          >
            <input
              className="input"
              type="number"
      ...
      <Section title="3 · Inventory">
        ...
        >
          ...
          <Field
            ...
            hint="Tracked per variant too (below on the product page)."
          >
            <input
              className="input"
              type="number"
          ...
          >
            <input
              ...
              defaultChecked={product?.allowBackorders ?? false}
              disabled={pending}
            />{" "}
            Allow backorders
          </label>
          ...
          >
            <input
              ...
              defaultChecked={product?.inventoryTracking ?? true}
              disabled={pending}
            />{" "}
            Track inventory
          </label>
      ...
      <Section title="4 · Images">
        <p className="muted" style={{ fontSize: "0.75rem", marginBottom: 8 }}>
          Uploaded images are stored in Cloudinary under this product&apos;s own
          folder. The main image is the shop card &amp; OG image.
        </p>
        <div
        ...
        {uploadError && (
          <p style={{ fontSize: "0.8rem", color: "#e5484d" }} role="alert">
            {uploadError}
          </p>
        )}
        {images.length === 0 ? (
          <div className="empty">
            <b>No images yet</b>Upload files or add an image URL.
          </div>
        ) : (
          images.map((img, i) => (
            <div
        ...
        <input
          type="hidden"
          name="uploadedAssets"
          value={JSON.stringify(uploadedAssets)}
        />
        ...
        >
          <Field label="Features" hint="One per line.">
            <textarea
              className="textarea"
              name="features"
              defaultValue={featuresLines.join("\n")}
              disabled={pending}
            />
          </Field>
          <Field label="What's included" hint="One per line.">
            <textarea
              className="textarea"
              name="whatsIncluded"
              defaultValue={product?.whatsIncluded?.list?.join("\n") ?? ""}
              disabled={pending}
            />
          </Field>
      ...
      <Section title="6 · Specifications">
        <Field
          ...
          hint="One per line as Key: value (e.g. Layout: 60% / Mounting: Gasket)."
        >
          <textarea
            className="textarea"
            name="specs"
            defaultValue={specsLines.join("\n")}
            disabled={pending}
          />
        </Field>
      </Section>
      ...
      <Section title="7 · Key features">
        <input
          type="hidden"
          name="cardFeatures"
          value={JSON.stringify(cardFeats)}
        />
        <p className="muted" style={{ fontSize: "0.72rem", margin: 0 }}>
          Add up to {MAX_CARD_FEATURES} key features shown on the shop product
        ...
        {cardFeats.map((f, i) => (
          ...
          >
            <span className="muted" style={{ fontSize: "0.7rem", width: 58 }}>
              Feature {i + 1}
            </span>
            <select
              className="select"
              value={f.icon}
              onChange={(e) => setFeat(i, { icon: e.target.value })}
              disabled={pending}
              style={{ width: 130 }}
            ...
            <input
              ...
              onChange={(e) => setFeat(i, { label: e.target.value })}
              disabled={pending}
              style={{ flex: "1 1 140px" }}
              aria-label={`Feature ${i + 1} label`}
            />
            <input
              className="input"
              value={f.value}
              maxLength={60}
              placeholder="Value (e.g. Focus Pro 30K)"
              onChange={(e) => setFeat(i, { value: e.target.value })}
              disabled={pending}
              style={{ flex: "1 1 160px" }}
              aria-label={`Feature ${i + 1} value`}
            />
            <button
              type="button"
              className="btn-admin sm"
              onClick={() => moveFeat(i, -1)}
              disabled={pending || i === 0}
              aria-label="Move up"
            >
              ↑
            ...
            <button
              ...
              className="btn-admin sm"
              onClick={() => moveFeat(i, 1)}
              disabled={pending || i === cardFeats.length - 1}
            ...
            <button
              ...
              className="btn-admin sm danger"
              onClick={() =>
                setCardFeats((rows) => rows.filter((_, j) => j !== i))
              }
              disabled={pending}
      ...
      <Section title="8 · Product configuration">
        ...
        <p className="muted" style={{ fontSize: "0.72rem", margin: 0 }}>
          Up to 3 option groups (e.g. Case Color, Switch Choice). Customer price
          = base price + add-ons. Leave empty for a simple product.
        </p>
        {optGroups.map((g, gi) => (
          <div
            key={gi}
          ...
          >
            ...
            >
              ...
              <input
                ...
                value={g.name}
                maxLength={60}
                placeholder="Group name (e.g. Case Color)"
              ...
              <button
                ...
                className="btn-admin sm danger"
                onClick={() =>
                  setOptGroups((rows) => rows.filter((_, j) => j !== gi))
                }
                disabled={pending}
              >
                Remove group
            ...
            {g.options.map((o, oi) => (
              ...
              >
                <input
                  ...
                  value={o.name}
                  maxLength={60}
                  placeholder="Option (e.g. Brass)"
                  onChange={(e) => setOption(gi, oi, { name: e.target.value })}
                  disabled={pending}
                  style={{ flex: "1 1 140px" }}
                  aria-label={`Group ${gi + 1} option ${oi + 1} name`}
                />
                <input
                  className="input"
                  type="number"
                  ...
                  step="1"
                  placeholder="0"
                  onChange={(e) => setOption(gi, oi, { addon: e.target.value })}
                  disabled={pending}
                  style={{ width: 120 }}
                  aria-label={`Group ${gi + 1} option ${oi + 1} add-on price in rupees`}
                />
                <span className="muted" style={{ fontSize: "0.7rem" }}>
                  ₹ add-on
                </span>
      ...
      <Section title="9 · Shipping">
        <p className="muted" style={{ fontSize: "0.75rem", marginBottom: 8 }}>
          Used to calculate Delhivery shipping rates. Weight is required for
          paid-shipping products; dimensions enable volumetric-weight pricing.
        </p>
        <div
          className="admin-grid cols-2"
          style={{
        ...
        >
          ...
          <Field label="Shipping restrictions">
            <input
              ...
              defaultValue={product?.shippingRestrictions ?? ""}
              disabled={pending}
            />
          </Field>
          <label
      ...
      <Section title="10 · SEO">
        ...
        >
          ...
          <Field
            ...
            hint="Defaults to the primary image."
          >
            <input
              className="input"
              name="ogImageUrl"
