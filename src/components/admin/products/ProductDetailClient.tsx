  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span
          className={`badge ${isActive ? "badge-lime" : isArchived ? "badge-err" : status === "DRAFT" ? "badge-warn" : "badge"}`}
        >
...
export function VariantsManager({
  ...
  return (
    ...
    >
      ...
      {variants.length === 0 ? (
        <div className="empty">
          <b>No variants yet</b>Add variants for options like colour, switch
          type or layout.
        </div>
      ) : (
          ...
          <tbody>
            {variants.map((v) => (
              <tr key={v.id}>
                ...
                <td className="num">
                  {v.price != null ? formatINR(v.price) : "Inherit"}
                </td>
                <td className="num">{v.stock}</td>
                <td className="num">
                  {Math.max(0, v.stock - v.reservedQuantity)}
                </td>
                <td>
                        ...
                        <>
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              if (
                                !confirm(
                                  `Delete variant "${v.name}"? Products in carts/orders keep their snapshot.`,
                                )
                              )
                                e.preventDefault();
                            }}
                          >
                            {pending ? <Spinner light /> : "Delete"}
                          </button>
                        </>
                      )}
                    </ActionForm>
          ...
          {(pending) => (
            ...
            >
              ...
              <input
                className="input"
                name="name"
                placeholder="Name (e.g. Black linear)"
              ...
              <input
                ...
                step="1"
                name="price"
                placeholder="Price ₹ (blank = inherit)"
                defaultValue={editing?.price != null ? editing.price / 100 : ""}
                min={0}
                disabled={pending}
              />
              <input
              ...
              <input
                className="input"
                name="options"
                placeholder="Options (Color: Black)"
