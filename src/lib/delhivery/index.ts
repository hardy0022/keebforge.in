...
export function parseTrackingResponse(httpStatus: number, bodyText: string): TrackingResult {
  ...
  if (httpStatus < 200 || httpStatus >= 300) {
    console.error(`[tracking] delhivery error status=${httpStatus}`, bodyText.slice(0, 300));
  ...
  const first = arr(ship)[0] && typeof arr(ship)[0] === "object" ? rec(arr(ship)[0]) : null;
  if (!first) {
    console.error("[tracking] no ShipmentData for waybill:", bodyText.slice(0, 300));
    return fail("UPSTREAM_ERROR");
  }
...
export async function trackShipment(waybill: string): Promise<TrackingResult> {
  ...
  if (!trackWaybillValid(waybill)) {
    return { ok: false, errorCode: "INVALID_PINCODE", message: SHIPPING_ERROR_MESSAGES.INVALID_PINCODE };
  }
...
export function parseBulkWaybills(httpStatus: number, bodyText: string): WaybillsResult {
  if (httpStatus < 200 || httpStatus >= 300) {
    console.error(`[waybill] delhivery error status=${httpStatus}`, bodyText.slice(0, 300));
    if (httpStatus === 401 || httpStatus === 403) return wbFail("INVALID_CREDENTIALS");
...
/** Pre-fetches N waybills from Delhivery (admin: allocate before manifestion). */
export async function fetchBulkWaybills(count = 1): Promise<WaybillsResult> {
  if (!process.env.DELHIVERY_API_TOKEN) {
    console.error("[waybill] DELHIVERY_API_TOKEN not configured");
    return wbFail("NOT_CONFIGURED");
  }
  const n = Math.max(1, Math.min(50, Math.floor(count)));
  try {
    const res = await fetch(`${BASE_URL}/waybill/api/bulk/json/?count=${n}`, {
      headers: { Authorization: `Token ${process.env.DELHIVERY_API_TOKEN}`, Accept: "application/json" },
      signal: AbortSignal.timeout(10_000),
    });
    return parseBulkWaybills(res.status, await res.text());
  } catch (err) {
    console.error("[waybill] Delhivery request failed:", err);
    return wbFail("UPSTREAM_ERROR");
  }
}
...
export function parseExpectedTat(httpStatus: number, bodyText: string, mot: "E" | "S"): ExpectedTatResult {
  if (httpStatus < 200 || httpStatus >= 300) {
    console.error(`[tat] delhivery error status=${httpStatus}`, bodyText.slice(0, 300));
    if (httpStatus === 401 || httpStatus === 403) return tatFail("INVALID_CREDENTIALS");
  ...
  const days = typeof raw === "number" ? raw : Number(String(raw ?? "").replace(/[^\d.]/g, ""));
  return {
    ok: true,
    mot,
...
/**
 * Expected delivery TAT between two pincodes at a mode. Caller passes
 * `expected_pickup_date` (defaults to now) and a `pdt` (product) hint.
 */
export async function expectedTat(opts: {
  originPincode: string;
  destinationPincode: string;
  mode: ShippingMode;
  pdt?: string;
  ...
  if (!process.env.DELHIVERY_API_TOKEN) {
    console.error("[tat] DELHIVERY_API_TOKEN not configured");
    return tatFail("NOT_CONFIGURED");
  }
  if (!isValidPincode(opts.originPincode) || !isValidPincode(opts.destinationPincode)) {
    return tatFail("INVALID_PINCODE");
  }
  const mot = opts.mode === "surface" ? "S" : "E";
  const params = new URLSearchParams({
    origin_pin: opts.originPincode,
    destination_pin: opts.destinationPincode,
    mot,
    pdt: opts.pdt ?? "",
    expected_pickup_date: opts.expectedPickupDate?.toString() ?? new Date().toString(),
  });
  try {
    const res = await fetch(`${BASE_URL}/api/dc/expected_tat?${params.toString()}`, {
      headers: { Authorization: `Token ${process.env.DELHIVERY_API_TOKEN}`, Accept: "application/json" },
      signal: AbortSignal.timeout(10_000),
    });
    return parseExpectedTat(res.status, await res.text(), mot);
  } catch (err) {
    console.error("[tat] Delhivery request failed:", err);
    return tatFail("UPSTREAM_ERROR");
  }
}
...
export function buildCreateShipmentBody(input: CreateShipmentInput): string {
  ...
  const shipment = {
    ...
    return_country: clean(input.pickup.returnCountry ?? input.pickup.country ?? "India"),
    products_desc: clean(input.productsDesc),
    hsn_code: "",
    cod_amount: input.paymentMode === "COD" ? (input.codAmountRupees ?? "0") : "0",
...
export function parseCreateShipmentResponse(httpStatus: number, bodyText: string): CreateShipmentResult {
  if (httpStatus < 200 || httpStatus >= 300) {
    console.error(`[shipment] delhivery cmu error status=${httpStatus}`, bodyText.slice(0, 300));
    if (httpStatus === 401 || httpStatus === 403) return createFail("INVALID_CREDENTIALS");
  ...
  // `shipments[].waybill`.
  for (const s of arr(rec(parsed).packages).concat(arr(rec(parsed).shipments))) {
  ...
  console.error("[shipment] cmu response without waybill:", bodyText.slice(0, 300));
  return { ok: false, errorCode: "UPSTREAM_ERROR", message: rmk || SHIPPING_ERROR_MESSAGES.UPSTREAM_ERROR };
}
...
export async function createShipment(input: CreateShipmentInput): Promise<CreateShipmentResult> {
  ...
  if (!input.pickup.pin_code) {
    console.error("[shipment] no pickup pincode (DELHIVERY_PICKUP_PIN / DELHIVERY_ORIGIN_PINCODE)");
    return createFail("NOT_CONFIGURED");
  }
  try {
...
export function parsePickupRequest(httpStatus: number, bodyText: string): PickupRequestResult {
  if (httpStatus < 200 || httpStatus >= 300) {
    console.error(`[pickup] delhivery error status=${httpStatus}`, bodyText.slice(0, 300));
    if (httpStatus === 401 || httpStatus === 403) return pickupFail("INVALID_CREDENTIALS");
  ...
  if (s.error && String(s.status ?? "").toLowerCase() !== "success") {
    ...
    const msg = typeof s.remark === "string" ? s.remark : typeof s.pickup_error === "string" ? s.pickup_error : "Could not book pickup.";
    console.error("[pickup] delhivery refusal:", bodyText.slice(0, 300));
    return { ok: false, errorCode: "UPSTREAM_ERROR", message: msg };
  }
  const id = typeof s.pickup_id === "string" ? s.pickup_id : s.id && typeof s.id === "string" ? s.id : null;
  return { ok: true, pickupId: id, raw: bodyText };
}

/**
 * Books a Delhivery pickup at the warehouse for the given date/time window.
 * `pickupLocation` must match a warehouse name registered in your Delhivery
 * account (same as DELHIVERY_PICKUP_NAME).
 */
export async function bookPickup(opts: {
  ...
  const body = {
    ...
    pickup_location: opts.pickupLocation ?? process.env.DELHIVERY_PICKUP_NAME ?? "",
    expected_package_count: opts.expectedPackageCount ?? 1,
  };
  if (!body.pickup_location) {
    console.error("[pickup] pickup location not configured (DELHIVERY_PICKUP_NAME)");
    return pickupFail("NOT_CONFIGURED");
  }
  try {
...
export function parseWarehouseResponse(httpStatus: number, bodyText: string): WarehouseCreateResult {
  ...
  const rmk =
    ...
    "";
  const bad = httpStatus < 200 || httpStatus >= 300 || !parsed || rec.success === false || rec.error === true;
  if (bad) {
    console.error(`[warehouse] delhivery error status=${httpStatus}`, bodyText.slice(0, 300));
    if (httpStatus === 401 || httpStatus === 403) return { ok: false, message: "Invalid Delhivery token." };
    return { ok: false, message: rmk || "Could not register the warehouse with Delhivery." };
  }
  return { ok: true };
}
 ...
 */
export async function createDelhiveryWarehouse(input: PickupLocation): Promise<WarehouseCreateResult> {
  if (!process.env.DELHIVERY_API_TOKEN) {
 ...
 */
export async function editDelhiveryWarehouse(input: PickupLocation): Promise<WarehouseCreateResult> {
  if (!process.env.DELHIVERY_API_TOKEN) {
    console.error("[warehouse] DELHIVERY_API_TOKEN not configured");
    return { ok: false, message: "DELHIVERY_API_TOKEN not configured." };
  }
...
if (process.argv[1]?.endsWith("delhivery/index.ts") || process.argv[1]?.endsWith("delhivery/index.js")) {
  runSelfCheck();
}
...
function runSelfCheck() {
  ...
  const prE = calculateShippingParams({ destinationPincode: "500085", paymentMode: "Pre-paid", weightGrams: 500, mode: "express" });
  t(prE.path.includes("ss=Delivered") && prE.path.includes("cod=0"), "forward → ss=Delivered, cod=0");
  const prRto = calculateShippingParams({ destinationPincode: "500085", paymentMode: "Pre-paid", weightGrams: 500, mode: "express", quoteType: "rto", cod: true });
  t(prRto.path.includes("ss=RTO") && prRto.path.includes("cod=1"), "rto → ss=RTO, cod=1");
  const prDto = calculateShippingParams({ destinationPincode: "500085", paymentMode: "Pre-paid", weightGrams: 500, mode: "express", quoteType: "dto" });
  t(prDto.path.includes("ss=DTO") && prDto.path.includes("cod=0"), "dto → ss=DTO, cod=0");
  const prCod = calculateShippingParams({ destinationPincode: "500085", paymentMode: "COD", weightGrams: 500, mode: "express" });
  t(prCod.path.includes("pt=COD") && prCod.path.includes("cod=1"), "COD payment → cod=1 by default");
  ...
  // ── Tracking parser ────────────────────────────────────────────────────────
  const expectTrackOk = (r: TrackingResult, expStatus: string, scans: number, tag: string) =>
    t(r.ok && r.data.status === expStatus && r.data.scans.length === scans, `${tag} (got ${r.ok ? r.data.status : r.errorCode} / ${r.ok ? r.data.scans.length : 0})`);
  const expectTrackErr = (r: TrackingResult, code: ShippingErrorCode, tag: string) =>
    t(!r.ok && r.errorCode === code, `${tag} (got ${r.ok ? "ok" : r.errorCode})`);
    ...
    ShipmentData: [
      {
        ...
        Destination: "Jammu",
        Scans: [
          { ScanDetail: { ScanType: "Picked Up", ScannedLocation: "Jammu", ScanDateTime: "2026-08-30 10:00:00", Instructions: "Package picked" } },
          { ScanDetail: { ScanType: "In Transit", ScannedLocation: "Delhi", ScanDateTime: "2026-09-01 04:12:33", Instructions: "Reached hub" } },
        ],
      },
    ],
  });
  expectTrackOk(parseTrackingResponse(200, TRACK_BODY), "In Transit", 2, "tracking shape: status + scans (oldest-first)");
  {
    const r = parseTrackingResponse(200, TRACK_BODY);
    t(r.ok && r.data.awb === "49323510001061" && r.data.destination === "Jammu", "tracking: awb + destination captured");
    t(r.ok && r.data.scans[0].location === "Jammu" && r.data.scans[0].status === "Picked Up", "tracking: first scan parsed");
  }
  expectTrackErr(parseTrackingResponse(200, '{"ShipmentData":[]}'), "UPSTREAM_ERROR", "tracking: empty ShipmentData → upstream");
  expectTrackErr(parseTrackingResponse(200, '<html>err</html>'), "UPSTREAM_ERROR", "tracking: non-JSON → upstream");
  expectTrackErr(parseTrackingResponse(401, '{"detail":"Invalid token"}'), "INVALID_CREDENTIALS", "tracking: 401 → credentials");
  expectTrackErr(parseTrackingResponse(429, "{}"), "RATE_LIMITED", "tracking: 429 → rate limited");
  t(trackWaybillValid("49323510001061") === true && trackWaybillValid("abc123") === false, "waybill format gate");
  {
    const rNoScans = parseTrackingResponse(200, JSON.stringify({ ShipmentData: [{ AWB: "1", Status: "Delivered", Scans: [] }] }));
    t(rNoScans.ok && rNoScans.data.scans.length === 0 && rNoScans.data.status === "Delivered", "tracking: no-scans delivered still ok");
  }

  // ── Shipment creation payload + response ───────────────────────────────────
  const cmuInput: CreateShipmentInput = {
    consignee: { name: "SAKSHITH SHETTY", address: "Jeppu Bappal", pin: "575002", city: "Mangaluru", state: "Karnataka", country: "India", phone: "9008144809" },
    order: "151",
...
productsDesc: "keyboard",
    ...
    pickup: { name: "Hardy", add: "Simla", city: "", pin_code: "", country: "India", phone: "", returnAdd: "Simla R", returnPin: "181206", returnCity: "Jammu", returnState: "J&K", returnCountry: "India" },
  };
  {
    const body = buildCreateShipmentBody(cmuInput);
    t(body.startsWith("format=json&data="), "cmu: format=json&data= prefix");
    t(body.includes('"name":"SAKSHITH SHETTY"') && body.includes('"pin":"575002"'), "cmu: consignee fields encoded");
    t(body.includes('"shipping_mode":"Surface"') && body.includes('"weight":"877 gm"'), "cmu: shipment fields");
    t(body.includes('"shipment_length":"20"') && body.includes('"shipment_width":"8"'), "cmu: all three dimensions sent");
    t(body.includes('"return_pin":"181206"') && body.includes('"return_add":"Simla R"') && body.includes('"return_country":"India"'), "cmu: return address from pickup");
    t(body.includes('"return_state":"JK"'), "cmu: return_state cleaned of & (would break form parsing)");
    t(body.includes('"pickup_location"'), "cmu: pickup_location present");
    const jsonPart = body.slice(body.indexOf("data=") + 5);
    t(!/[&#%;\\]/.test(jsonPart), "cmu: no form-breaking chars anywhere in the data payload");
  }
  {
    const r = parseCreateShipmentResponse(200, JSON.stringify({ shipments: [{ name: "SAKSHITH SHETTY", waybill: "49323510001061" }] }));
    t(r.ok && r.waybill === "49323510001061", "cmu: waybill extracted");
  }
  {
    const r = parseCreateShipmentResponse(
      200,
      JSON.stringify({ package_count: 1, upload_wbn: "UPL11185518272788448003", packages: [{ waybill: "49323510001072", refnum: "KF46VSFP" }] })
    );
    t(r.ok && r.waybill === "49323510001072", "cmu: waybill from packages[]");
  }
  t(parseCreateShipmentResponse(200, JSON.stringify({ shipments: [] })).ok === false, "cmu: no waybill → error");
  t(parseCreateShipmentResponse(401, '{"detail":"Invalid token"}').ok === false, "cmu: 401 → credentials");
  t(parseCreateShipmentResponse(200, "<html>err").ok === false, "cmu: non-JSON → error");

  // ── Bulk waybill fetch ────────────────────────────────────────────────────
  {
    const r = parseBulkWaybills(200, JSON.stringify({ waybills: ["49323510001061", "49323510001062"] }));
    t(r.ok && r.waybills.length === 2 && r.waybills[0] === "49323510001061", "bulk waybills parsed");
  }
  t(parseBulkWaybills(200, JSON.stringify({ waybills: [] })).ok === false, "bulk: empty waybills → error");
  t(parseBulkWaybills(401, '{"detail":"Invalid token"}').ok === false, "bulk: 401 → credentials");
  ...
  {
    const r = parseExpectedTat(200, '{"tat_d2d":"3 Days","origin":"575002","destination":"575001"}', "S");
  ...
  {
    const r = parsePickupRequest(200, JSON.stringify({ error: true, status: "Failed", remark: "Warehouse not found" }));
    t(r.ok === false && r.message.includes("Warehouse"), "pickup: error shape → refusal with remark");
  }
  t(parsePickupRequest(401, '{"detail":"Invalid token"}').ok === false, "pickup: 401 → credentials");
  t(parsePickupRequest(200, "<html>err").ok === false, "pickup: non-JSON → error");

  // ── Client warehouse (pickup location) registration ────────────────────────
  t(parseWarehouseResponse(200, JSON.stringify({ success: true, name: "test_name" })).ok === true, "warehouse: 200 success");
  {
    const r = parseWarehouseResponse(200, JSON.stringify({ rmk: "Warehouse already exists", success: false }));
    t(!r.ok && /already/i.test(r.message ?? ""), "warehouse: success=false with rmk detected");
  }
  t(parseWarehouseResponse(400, '{"rmk":"ClientWarehouse matching query does not exist."}').ok === false, "warehouse: 400 rmk extracted");
  t(parseWarehouseResponse(401, '{"detail":"Invalid token"}').ok === false, "warehouse: 401 → credentials");
  t(parseWarehouseResponse(200, "<html>err").ok === false, "warehouse: non-JSON → error");
